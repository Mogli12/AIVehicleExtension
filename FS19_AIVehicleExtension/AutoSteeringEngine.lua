...
function AutoSteeringEngine.checkChain( vehicle, iRefNode, wheelBase, maxSteering, widthOffset, turnOffset, isInverted, useFrontPacker, useAIFieldFct )
	...
	if vehicle.articulatedAxis ~= nil and vehicle.articulatedAxis.rotMax ~= nil then
		vehicle.aiveChain.maxToolAngle = math.min( vehicle.aiveChain.maxToolAngle + 0.5 * vehicle.articulatedAxis.rotMax, AIVEGlobals.maxToolAngleA )
	end
...
function AutoSteeringEngine.checkTools1( vehicle, reset )
	...
	--			tool.aiArea.prohibitedFruitType      = g_fruitTypeManager.nameToIndex.UNKNOWN
...
function AutoSteeringEngine.initTools( vehicle, maxLooking, leftActive, widthOffset, headlandDist, collisionDist, turnMode )
	...
	if vehicle.aiveHas.sowingMachine then
		local currentSeed = g_fruitTypeManager.nameToIndex.UNKNOWN
		if vehicle.aiveHas.sowingMachine then
			for _,tool in pairs(vehicle.aiveChain.tools) do
				if tool.spec_sowingMachine ~= nil then
					currentSeed = tool.obj.spec_sowingMachine.seeds[tool.obj.spec_sowingMachine.currentSeed]
				end
			end
		end
		...
		if currentSeed == nil then
			...
			currentSeed = g_fruitTypeManager.nameToIndex.UNKNOWN
		end
		...
		for _,tool in pairs(vehicle.aiveChain.tools) do
			if tool.aiArea ~= nil and tool.aiArea.hasSowingChannel then
				...
				tool.aiArea.prohibitedMinGrowthState = 0
				if currentSeed ~= g_fruitTypeManager.nameToIndex.UNKNOWN then
					tool.aiArea.prohibitedMaxGrowthState = g_fruitTypeManager.indexToFruitType[currentSeed].maxHarvestingGrowthState
				else	
					tool.aiArea.prohibitedMaxGrowthState = 0
				end
			end
...
--	if vehicle.aiveChain.fbb3 ~= nil and AIVEUtils.vector2LengthSq( vehicle.aiveChain.fbb3.x - wx, vehicle.aiveChain.fbb3.z - wz ) < 0.01 then
...
function AutoSteeringEngine.hasFruitsInFront( vehicle )
	...
	if AIVEGlobals.useFBB123 > 0 then
		...
		if      vehicle.aiveChain.fbb2   ~= nil 
				...
				and AIVEUtils.vector2LengthSq( vehicle.aiveChain.fbb2.x - wx, vehicle.aiveChain.fbb2.z - wz ) < 0.09 then
...
--print("dt: "..tostring(dt)..", wantedSpeed: "..tostring(wantedSpeed).."("..tostring(speedLevel).."), acc:"..tostring(acc).."("..tostring(allowedToDrive)..")")
	...
	elseif math.abs( wantedSpeed - curSpeed ) < 0.5 then
		...
		vehicle.acLastWantedSpeed = vehicle.acLastWantedSpeed + AIVEUtils.clamp( wantedSpeed - vehicle.acLastWantedSpeed, -0.0015 * dt, 0.0015 * dt )
	end
	...
	if vehicle.aiveChain.headland > 0 then		
		...
		for j=-2,2 do
			...
			if AutoSteeringEngine.checkField( vehicle, x,z) then
				AIVEDrawDebugPoint( x,y,z	, 0, 1, 0, 1 )
			else
				AIVEDrawDebugPoint( x,y,z	, 1, 0, 0, 1 )
			end
			if vehicle.aiveChain.respectStartNode then
	...
	if AIVEGlobals.showChannels > 0 then
		...
		if vehicle.aiveTestMap ~= nil then
			for _,p in pairs( vehicle.aiveTestMap ) do
				...
				AIVEDrawDebugPoint( x, y, z, 1,1,1, 1 )
			end
		end
	end
...
function AutoSteeringEngine.addTool( vehicle, implement, ignore )
	...
	tool.configFileName          = "" --string.lower( AIVEUtils.removeModDirectory(object.configFileName) )
	tool.obj                     = object
	tool.isAITool                = false
	tool.specialType             = ""
	tool.b1                      = 0
