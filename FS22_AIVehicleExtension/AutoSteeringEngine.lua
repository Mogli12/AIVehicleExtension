...
function AutoSteeringEngine.getAiWorldPosition( vehicle )
	if      vehicle.acAiPos ~= nil
			...
			and vehicle.spec_aiFieldWorker.isActive then
...
if vehicle.aiveChain.angleBuffer ~= nil then AIVehicleExtension.debugPrint( vehicle, "Reset angle buffer 9") end 
		...
		if vehicle.aiveToolsDirty then	
			...
			vehicle.aiveChain.tools = {}
			for _,implement in pairs(vehicle.spec_aiFieldWorker.aiImplementList) do
				AutoSteeringEngine.addTool(vehicle,implement)
			end
...
function AutoSteeringEngine.getToolsSpeedLimit( vehicle )
	local c = vehicle:getCruiseControlMaxSpeed()
	local s, _  = vehicle:getSpeedLimit(true)
	return math.min( c, s )
end
...
-- precondition: vehicle.rotatedTime is filled from last steering
	...
	local diff = dt * 0.001 --vehicle.spec_aiVehicle.aiSteeringSpeed
...
function AutoSteeringEngine.getWorldTargetFromSteeringAngle( vehicle, angle, moveForwards )
	...
	local refNode = vehicle.aiveChain.refNode -- vehicle:getAIDirectionNode()
...
--print("dt: "..tostring(dt)..", wantedSpeed: "..tostring(wantedSpeed).."("..tostring(speedLevel).."), acc:"..tostring(acc).."("..tostring(allowedToDrive)..")")
	...
	if not allowedToDrive or wantedSpeed < 1 then 
		...
		if wantedSpeed > 1 and vehicle.acLastWantedSpeed < 2 then
			vehicle.acLastWantedSpeed = 2
		end
...
function AutoSteeringEngine.getAIAreaOfVehicle2( vehicle, toolIndex, lx1,lz1,lx2,lz2,lx3,lz3 )
	...
	local useDensityHeightMap = #tool.obj:getAIDensityHeightTypeRequirements() > 0

	if not useDensityHeightMap then
		local query, isValid = tool.obj:getFieldCropsQuery()
		...
		local densityHeightTypeRequirements = tool.obj:getAIDensityHeightTypeRequirements()

		return AIVehicleUtil.getAIDensityHeightArea(lx1,lz1,lx2,lz2,lx3,lz3, densityHeightTypeRequirements)
	end
...
--**********************************************************************************************		
		if vehicle.aiveCurrentField ~= nil then 
			local pN, pX, pZ = 0, 0, 0
			allPoints = vehicle.aiveCurrentField.getPoints()
			for _,p in pairs( allPoints ) do
				local x,z = unpack( p )
				pN = pN + 1
				pX = pX + x 
				pZ = pZ + z 
			...
			if pN > 0 then 
				...
				for _,p in pairs( allPoints ) do
					...
					x = x - pX 
					z = z - pZ  
					local d = math.deg( math.atan2( x, z ) ) 
					if d < 0 then 
						d = d + 180 
					end 
					local i = math.floor( d / 15 + 0.5 ) 
					local l = math.sqrt( x*x + z*z )
					if maxD[i] == nil or maxD[i] < l then 
						maxD[i] = l 
					end 
				end 
				...
				for i=0,12 do 
					if maxD[i] ~= nil then 
						if minL == nil or minL > maxD[i] then 
							minL = maxD[i] 
							minA = i * 15
						end 
						if maxL == nil or maxL < maxD[i] then 
							maxL = maxD[i] 
							maxA = i * 15
						end 
						print(string.format("%3.0f°: %5.1f", i * 15, maxD[i] ) )
					end 
				end 
				
				print( "Best direction: "..tostring(minA)..", "..tostring(maxA) ) 
				
				
				local minX, minA, maxZ, maxB = nil, -1, nil, -1
				...
				for i=0,24 do 
					...
					local dx1, dx2, dz1, dz2 = nil, nil, nil, nil 
					for _,p in pairs( allPoints ) do
						local x,z = unpack( p )
						x = x - pX 
						z = z - pZ  
					...
					if minX == nil or dx2 - dx1 < minX then 
						minX = dx2 - dx1
						minA = math.deg( a )
					end 
					if maxZ == nil or dz2 - dz1 > maxZ then 
						maxZ = dz2 - dz1
						maxB = math.deg( a )
					end 
				end 

				print( "Best direction: "..tostring(minA)..", "..tostring(maxB) ) 
